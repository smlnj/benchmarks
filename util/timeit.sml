...
structure Timing : sig
    ...
    (* time the running of a function; the runtime is printed to the output stream *)
    ...
    (* run a test function.  The first argument is the output file for the program
     * ("-" maps to stdOut) and the second argument is the function to run.
     *)
    val testIt : string * (unit -> unit) -> unit
    ...
    fun testIt ("-", f) = Log.withOutput TextIO.stdOut f ()
      | testIt (file, f) = let
          val outS = TextIO.openOut file
          in
            (Log.withOutput TextIO.stdOut f ())
