...
structure Rules =
  struct
    ...
    (* NOTE: The original version of the benchmark uses a "val" binding
     * here for this initialization, but that does not work when building
     * with CM, so I have converted this to an initialization function
     * that is called in the `Main` stucture.
     *)
    fun init () = (
add (CProp
("equal",
