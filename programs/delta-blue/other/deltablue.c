...
*/
/*
   Fixed various minor issues that modern C compilers complain about.
   John Reppy, University of Chicago, 2025
*/
...
{
  List_Do(v->constraints, (Proc)AddIfSatisfiedInput);
}
...
List ExtractPlan(void)
{
  ...
  List_RemoveAll(hot);
  List_Do(allVariables, (Proc)CollectSatisfiedInputs);
  return MakePlan();
}
...
List ExtractPlanFromConstraints(List constraints)
{
  if (hot == NULL) hot = List_Create(128);
  List_RemoveAll(hot);
  List_Do(constraints, (Proc)AddIfSatisfiedInput);
  return MakePlan();
}
...
void IncrementalRemove(Constraint c)
{
  ...
  for (strength = S_required; strength <= S_weakest; strength++) {
    List_Do(unsatisfied, (Proc)AddAtStrength);
  }
...
void RemovePropagateFrom(Variable v)
{
  ...
  todo = List_Create(8);
  while (true) {
    List_Do(v->constraints, (Proc)CollectUnsatisfied);
...
Constraint EqualsC(Variable a, Variable b, Strength strength)
{
  ...
  new->execute = (Proc)EqualsC_Execute;
  new->variables[0] = a;
  new->variables[1] = b;
  new->methodCount = 2;
  new->methodOuts[0] = 0;
			...
			Variable dest, Strength strength)
{
  ...
  new->execute = (Proc)ScaleOffsetC_Execute;
  new->variables[0] = src;
